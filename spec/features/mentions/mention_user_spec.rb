feature 'mention user' do
  ...
  scenario 'user can mention muliple users in a peep' do
    sign_in
    ...
    expect(first('.peep-body')).to have_content 'test_username mentioned you in a peep!'
    expect(first('.peep-body')).to have_content 'I am mentioning @test_username2 and @test_username3 in a peep'

    click_button('sign_out')

    expect(current_path).to eq '/'

    fill_in 'email', with: 'test@testmail3.com'
    fill_in 'password', with: 'password123'
    click_button('Sign In')

    expect(current_path).to eq '/peeps'

    expect(page).to have_content('Signed in as, test_username3')
    click_link('test_username3')
