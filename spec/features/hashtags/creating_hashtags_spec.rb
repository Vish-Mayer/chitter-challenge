...
feature 'creating hashtags' do
  ...
  scenario 'user can create multiple tag in a peep by putting a "#" at the start of a word' do

    expect(current_path).to eq '/peeps'

    fill_in 'text_area', with: 'this is a test #hashtag #chitter'
    click_button('Peep')
    expect(first('.peep')).to have_content('this is a test #hashtag')
    expect(first('.peep')).to have_selector(:link_or_button, '#hashtag')
    expect(first('.peep')).to have_selector(:link_or_button, '#chitter')
  end
end
